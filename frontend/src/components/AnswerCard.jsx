...
export default function AnswerCard({
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl2 border transition-all ${
      isAccepted 
        ? "border-emerald-200 bg-emerald-50/30 ring-1 ring-emerald-100" 
        : "border-gray-100 bg-white shadow-sm hover:shadow-md"
    }`}>
      
      {/* Accepted Badge (Absolute Positioned) */}
      {isAccepted && (
      ...
      <div className="flex gap-4 p-5">
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <span className={`text-lg font-bold ${answer.vote_score > 0 ? "text-brand-accent" : "text-gray-600"}`}>
            {answer.vote_score ?? 0}
          </span>
          ...
          <div className="mt-4 flex items-center justify-between border-t border-gray-100 pt-3">
             ...
             {canAccept && !isAccepted && (
              <button
                ...
                className="group flex items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-600 transition-all hover:bg-emerald-500 hover:text-white"
              >
