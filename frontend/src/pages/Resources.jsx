  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log(`Fetching resources for subject ${selection.subjectId} with type ${selectedType} for user ${token ? headers : "guest"}`);
        console.log(headers);
        console.log(`API URL: ${API_BASE_URL}/api/resources?subjectId=${selection.subjectId}&type=${selectedType}`);
    ...
    <div className="min-h-screen bg-background-light py-8 font-sans text-brand-deep">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <h1 className="text-3xl font-extrabold text-brand-deep">Study Library</h1>
            <p className="mt-2 text-brand-mid/80">
              Access curated notes, past papers, and lectures for your specific course.
            </p>
          </div>
          <Link 
            to="/contact" 
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-4">
          ...
          <div className="lg:col-span-1">
             <div className="sticky top-8 space-y-2">
                <h3 className="mb-3 px-2 text-xs font-bold uppercase text-gray-400">Resource Type</h3>
                  ...
                  {RESOURCE_TYPES.map((t) => {
                    ...
                    return (
                      <button
                        ...
                        `}
                      >
                        <Icon className={`h-5 w-5 ${isActive ? "text-brand-accent" : "text-gray-400"}`} />
                        {t.label}
                      </button>
                    );
                  })}
          ...
          <div className="lg:col-span-3">
            {!canShowContent ? (
              <div className="flex min-h-[400px] flex-col items-center justify-center rounded-xl2 border border-dashed border-gray-300 bg-white/50 text-center">
                ...
                <h3 className="text-lg font-bold text-brand-deep">Select a subject first</h3>
                <p className="max-w-xs text-sm text-gray-500">
                  Choose your university, degree, and semester above to unlock the library.
                </p>
              </div>
            ) : loading ? (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                 {[1,2,3,4].map(i => (
                    <div key={i} className="h-32 w-full animate-pulse rounded-xl2 bg-white/60"></div>
                 ))}
              </div>
            ) : resources.length === 0 ? (
              <div className="rounded-xl2 border border-orange-100 bg-orange-50/50 p-8 text-center">
                 ...
                 <h3 className="text-lg font-bold text-brand-deep">No {selectedType} found</h3>
                 <p className="text-sm text-gray-600">
                   We don't have materials for this section yet.
                 </p>
                 <Link to="/contact" className="mt-4 inline-block text-sm font-bold text-brand-accent hover:underline">
...
function ResourceCard({ resource }) {
  ...
  const isLink = !resource.url?.endsWith(".pdf") && !resource.url?.endsWith(".docx");
  ...
  const handleToggleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/resources/${resource.id}/save`, {
        method: "POST",
        headers: {
    ...
    <div className="group flex flex-col justify-between rounded-xl2 border border-gray-100 bg-white p-5 shadow-sm transition-all hover:-translate-y-1 hover:shadow-md">
      <div>
        <div className="mb-3 flex items-start justify-between">
          <div className={`rounded-lg p-2 ${isLink ? 'bg-blue-50 text-blue-500' : 'bg-red-50 text-red-500'}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className={`rounded-lg p-1.5 transition-colors hover:bg-gray-100 ${
                isSaved ? "text-brand-accent" : "text-gray-300 hover:text-gray-500"
              }`}
        ...
        <div className="mt-2 flex items-center gap-2 text-xs text-gray-400">
           <span>{resource.difficulty ? `Difficulty: ${resource.difficulty}/5` : "General"}</span>
           <span>•</span>
           <span>English</span>
        </div>
      </div>
      ...
      <div className="mt-5 border-t border-gray-100 pt-4">
        {resource.url ? (
          ...
          >
            ...
            {isLink ? <ExternalLink className="h-3 w-3" /> : <Download className="h-3 w-3" />}
          </a>
        ) : (
          <button disabled className="w-full cursor-not-allowed rounded-lg bg-gray-100 py-2 text-sm font-bold text-gray-400">
            Link Unavailable
          </button>
        )}
      </div>
